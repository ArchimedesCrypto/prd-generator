...
export default function BlogTag() {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      ...
      try {
        ...
        const result = await client.getTagArticles(slug, page, 10);
  ...
  return (
    <div className="min-h-screen parchment-bg">
      {slug && (
        <SEO 
          title={`${deslugify(slug)} Tag - Deepwriter AI Blog`}
          description={`Browse articles tagged with ${deslugify(slug)}. Learn more about AI document generation with Deepwriter AI.`}
          canonical={`https://yourdomain.com/blog/tag/${slug}`}
        />
      )}
      <div className="max-w-4xl my-8 lg:mt-10 mx-auto px-4 md:px-8">
        ...
        <p className="text-stone-gray-light mb-8">Articles tagged with "{slug && deslugify(slug)}"</p>
        ...
        {!loading && lastPage > 1 && slug && (
          <Pagination basePath={`/blog/tag/${slug}`} pageNumber={page} lastPage={lastPage} />
        )}
      </div>
    </div>
  );
